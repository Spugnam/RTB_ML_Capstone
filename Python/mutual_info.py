'''
Downloaded from https://gist.github.com/GaelVaroquaux/ead9898bd3c973c40429#file-mutual_info-py
...
mutual_info.mutual_information_2d(test_df['clicked'].ravel(),
                                  test_df['clicked'].ravel())  # same as sklearn

MI_list = list()
for var in test_df.select_dtypes(include=['int64']).columns:  # loop over numeric columns
    MI_list.append([var,
                    mutual_info.mutual_information_2d(test_df[var].values,
                                                      test_df['clicked'].values)])

MI_list.sort(key=lambda x: -x[1])  # sort by MI
for var, MI in MI_list:
    print(var, MI)
